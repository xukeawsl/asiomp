...
std::shared_ptr<session> session_factory::default_creator(
    asio::ip::tcp::socket socket) {
    return std::make_shared<session>();
}

void session_factory::register_session(const std::string& session_name,
                                       session_creator creator) {
    this->creators_[session_name] = creator;
}

std::shared_ptr<session> session_factory::create_session(
    const std::string& session_name, asio::ip::tcp::socket socket) {
