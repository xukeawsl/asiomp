...
session::session(asio::ip::tcp::socket new_socket)
    : executor(new_socket.get_executor()),
      socket(std::move(new_socket)),
      dst_socket(executor),
      udp_resolver(executor),
      deadline(executor) {
...
void session::check_deadline() {
    ...
    if (deadline.expiry() <= asio::steady_timer::clock_type::now()) {
        ...
        deadline.async_wait(std::bind(&session::check_deadline, self));
    }
}

void session::set_timeout(size_t second) { this->timeout = second; }
...
void session::get_version_and_nmethods() {
...
std::string session::methods_toString() {
...
SocksV5::Method session::choose_method() {
    for (auto&& method : this->methods) {
...
void session::reply_support_method() {
...
void session::do_no_auth() { this->get_request_from_client(); }

void session::do_username_password_auth() { this->get_username_length(); }

void session::get_username_length() {
    std::array<asio::mutable_buffer, 2> buf = {
...
void session::get_username_content() {
    auto self = shared_from_this();
    asio::async_read(
...
void session::get_password_length() {
...
void session::get_password_content() {
    auto self = shared_from_this();
    asio::async_read(
...
void session::do_auth_and_reply() {
    if ("user" == std::string(this->uname.begin(), this->uname.end()) &&
        "passwd" == std::string(this->passwd.begin(), this->passwd.end())) {
...
void session::get_request_from_client() {
...
void session::get_dst_information() {
...
void session::resolve_domain() { this->resolve_domain_length(); }

void session::resolve_domain_length() {
...
void session::resolve_domain_content() {
    std::array<asio::mutable_buffer, 2> buf = {
        {asio::buffer(this->dst_addr.data(), this->dst_addr.size()),
         asio::buffer(&this->dst_port, 2)}};
...
void session::set_connect_endpoint() {
    switch (this->request_atyp) {
        case SocksV5::RequestATYP::Ipv4: {
...
void session::set_udp_associate_endpoint() {
    switch (this->request_atyp) {
        case SocksV5::RequestATYP::Ipv4: {
...
void session::async_udp_dns_reslove() {
    auto self = shared_from_this();
...
void session::reply_udp_associate() {
...
bool session::check_sender_endpoint() {
...
void session::parse_udp_message() {
...
void session::async_send_udp_message() {
    auto self = shared_from_this();
    this->udp_resolver.async_resolve(
...
void session::try_to_send_by_iterator(
...
void session::send_udp_to_client() {
...
void session::receive_udp_message() {
    auto self = shared_from_this();
    this->udp_socket->async_receive_from(
        asio::buffer(this->client_buffer.data(), this->client_buffer.size()),
        this->sender_endpoint,
...
void session::async_dns_reslove() {
    auto self = shared_from_this();
    this->udp_resolver.async_resolve(
        convert::dst_to_string(this->dst_addr, ATyp::DoMainName),
        std::to_string(this->dst_port),
...
void session::try_to_connect_by_iterator(
    asio::ip::udp::resolver::results_type::const_iterator iter) {
    if (iter == this->resolve_results.end()) {
...
void session::reply_and_stop(SocksV5::ReplyREP rep) {
    this->rep = rep;
    this->reply_atyp = SocksV5::ReplyATYP::Ipv4;
...
void session::reply_connect_result() {
    std::array<asio::const_buffer, 6> buf = {
        {asio::buffer(&this->ver, 1), asio::buffer(&this->rep, 1),
         asio::buffer(&this->rsv, 1), asio::buffer(&this->reply_atyp, 1),
         asio::buffer(this->bnd_addr.data(), this->bnd_addr.size()),
...
void session::send_to_dst(size_t write_length) {
    auto self = shared_from_this();
    asio::async_write(
        this->dst_socket,
...
void session::send_to_client(size_t write_length) {
    auto self = shared_from_this();
    asio::async_write(
